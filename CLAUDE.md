...
| `/init-elektra` | Init | Run or re-run onboarding | `init-elektra/SKILL.md` |
...
**Trigger patterns:** "godspeed" / "plan and execute" / "let's build" / "fix this" -> SO1. "check issues" / "project status" -> SO2. "quality check" / "design review" -> SO3 + `/plan-design-review` + `/ui-ux-pro-max`. "responsible AI" / "RAI" / "compliance" -> RAI. "iterate" / "improve" -> Autoresearch. "ship it" / "deploy" -> Ship. "debug" / "investigate" -> Debug. "review plan" / "plan review" -> P3.5 gate. "update elektra" / "upgrade elektra" / "get latest version" -> `/elektra-update`. "onboard" / "re-onboard" / "reset onboarding" / "init elektra" -> `/init-elektra`.
